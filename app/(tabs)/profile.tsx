    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={styles.modalOverlay} onPress={onClose}>
        <Pressable style={styles.editModal} onPress={e => e.stopPropagation()}>
          ...
          {/* Avatar - UPDATED with new Senya character */}
...
const MOCK_PROFILE_DATA = {
  ...
  learning_path: {
    learning_goal: 'Everything',
    practice_time: '30_min'
  }
};
...
export default function Profile() {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      // Set learning path from mock data
  ...
  const useMockData = (xp: number) => {
    ...
    const completedLessons = MOCK_PROFILE_DATA.lessons.filter(l => l.status === 'completed');
    setTotalLessons(completedLessons.length);

    // Calculate badges
    const earnedBadges = Math.min(Math.floor(xp / 50) + 1, 8);
    ...
    const progressItems = MOCK_PROFILE_DATA.lessons.slice(0, 4).map((lesson, index) => {
      const pct = lesson.status === 'completed' ? 100 :
        lesson.status === 'in_progress' ? 50 : 0;
      return {
        name: lesson.title,
        pct: pct,
