...
// ── GRADIENT BACKGROUND COLORS (THE PRETTY ONE!) ─────────────────────
const GRADIENT = {
  start: '#87CEEB',
  mid: '#B3E5FC',
  mid2: '#E3F2FD',
...
const CHARACTERS = {
  senya: require('../../assets/images/new_characters/senya.png'),
...
const COLORS = {
  ...
  floraPink: '#FF6EB4',
  floraPinkGlow: 'rgba(255, 110, 180, 0.15)',
  cattoSky: '#4FC3F7',
...
const CHARACTER_MESSAGES = {
  senya: [
    ...
    "I believe in you! 💪",
    "You're a signing superstar! ⭐",
  ],
  flora: [
    "🌸 You've got this!",
    "Learning is beautiful, just like you! 💖",
    "Every sign you learn opens a new door! 🚪",
    "You're blooming into a great signer! 🌺",
  ],
  catto: [
    "😺 Meow-velous progress!",
    "Paws-itively amazing! 🐾",
    "You're the cat's meow! 🌟",
    "Purr-fect job, keep going! 🐱",
  ],
};

function getCharacterMessage(character: keyof typeof CHARACTER_MESSAGES): string {
  const messages = CHARACTER_MESSAGES[character];
...
const MOCK_TEACHER_LESSONS = [
  {
    lesson_id: 1,
    title: "FSL Alphabet - Basic Signs",
    description: "Learn the fundamental Filipino Sign Language alphabet from A to Z.",
    lesson_type: "video",
    difficulty: "Beginner",
    status: "completed",
  ...
  },
  {
    ...
    description: "Master common greetings like 'Hello', 'Good morning', and introductions.",
  ...
  {
    lesson_id: 3,
    title: "Numbers 1-20 in FSL",
    description: "Learn how to sign numbers from 1 to 20 with proper hand positions.",
    lesson_type: "video",
    difficulty: "Beginner",
    status: "in_progress",
  ...
  {
    lesson_id: 4,
    title: "Classroom Words and Phrases",
    description: "Essential signs for the classroom setting - teacher, student, book, etc.",
    lesson_type: "interactive",
  ...
  {
    lesson_id: 5,
    title: "Family Members in FSL",
    description: "Signs for family members - mother, father, sibling, grandparents.",
    lesson_type: "video",
    difficulty: "Intermediate",
    status: "pending",
    assigned_at: "2026-06-23T08:00:00Z",
    has_quiz: false,
    total_steps: 10,
    progress: null
  }
...
export default function Lessons() {
  ...
  // Rotate characters every 8 seconds
  useEffect(() => {
    const characters: (keyof typeof CHARACTER_MESSAGES)[] = ['senya', 'flora', 'catto'];
    let index = 0;
    ...
    const interval = setInterval(() => {
      ...
      setCharacterMessage(getCharacterMessage(char));
    }, 8000);

    // Set initial message
    setCharacterMessage(getCharacterMessage('senya'));

    return () => clearInterval(interval);
  }, []);

  // ── Load lessons from mock data ──
  const loadLessonsData = async () => {
    try {
      ...
      if (userData) {
        ...
        if (user.student) {
          setStreak(user.student.streak_days || 12);
          setXp(user.student.total_xp || 150);
        }
      }
  ...
  const navigateToLesson = (lessonId: number) => {
    router.push(`/quiz/mc?lessonId=${lessonId}` as Href);
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        {/* Animated Gradient Background - THE PRETTY ONE! */}
        ...
        <Animated.View style={[styles.mapContainer, { opacity: tabFadeAnim }]}>
          {activeTab === 1 && loadingTeacher ? (
            ...
            >
              ...
              {/* Rotating Character with Frosted Glass Bubble */}
              {activePos && (
                <Animated.View
                  style={[
                ...
                >
                  <Image
                    source={CHARACTERS[currentCharacter]}
                    style={styles.mascotImage}
                    contentFit="contain"
                  />
                  {/* Frosted Glass Speech Bubble */}
                  <View style={styles.frostedBubble}>
                    <View style={styles.frostedBubbleGlow} />
                    <Text style={styles.frostedBubbleText}>{characterMessage}</Text>
                  </View>
                </Animated.View>
              )}
        ...
        {/* Floating Detail Sheet - KEPT EXACTLY THE SAME */}
        {selectedLesson && (
...
const styles = StyleSheet.create({
  ...
  // ── Character with Frosted Glass Bubble ──
  mascotContainer: {
    position: 'absolute',
    alignItems: 'center',
    zIndex: 12,
  ...
  frostedBubble: {
    ...
    shadowColor: 'rgba(46, 127, 232, 0.08)',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.1,
    shadowRadius: 12,
    elevation: 4,
