...
export default function WebViewCameraScreen() {
    ...
    const startSimulation = () => {
        ...
        detectionIntervalRef.current = setInterval(() => {
            ...
            // Randomly decide if the user gets it right or wrong
            // Start with 70% correct, gradually gets harder as we progress
            const progress = completedLetters.size / ALPHABET_PART1.length;
            const accuracy = 0.9 - (progress * 0.3); // 90% at start, 60% at end
            const isCorrect = Math.random() < accuracy;

            // Pick a letter to detect
            let detected = '';
            if (isCorrect) {
                detected = target;
            } else {
                // Pick a wrong letter from the alphabet (could be completed or not)
                const possibleWrong = ALPHABET_PART1.filter(l => l !== target);
        ...
        }, 1200 + Math.random() * 800); // Random interval between 1.2-2s
    };
